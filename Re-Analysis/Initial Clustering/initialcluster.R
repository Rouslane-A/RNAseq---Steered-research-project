### Initial clustering of all cells based on cell origins and model
#load in relevant libraries
library(scater)
library(scuttle)

# Read csv count matrix as dataframe - assuming data is in working directory
...
# Categorize column names based on expressions (cell model)
...
categorize_columns <- function(col_name) {
  if (grepl("Metastases", col_name)) {
    return("Lung")
  } else if (grepl("Tumor", col_name)) {
    return("Tumor")
  } else if (grepl("Lung", col_name)) {
    return("Lung")
  } else {
    return("Other")
  }
...
# Add column_categories_factor to colData
colData(sce)$origin <- column_categories_factor

# Plot t-SNE plot based on cell origin 
plotTSNE(sce, colour_by = "origin")
